...
Your Client ID: 7cd68f4075a54e28b2751024275f0563


REST API OAuth 1.0 Credentials:
Your Consumer Key: 7cd68f4075a54e28b2751024275f0563
Your Consumer Secret: 11960e03f1404566a65cd0879f13062d
"""
...
def get_info_cup_of(food_name, union_n):
    ...
    sum_of_nutritional(result, union_n)


def elements_parsing(food_dict):
    """Перевод значений ключа из строки в значения"""
...
def sum_of_nutritional(food, union_n):
    """Сумирование значние общего словаря"""
    for name in LIST_OF_ELEMENTS[:-2]:
        try:
            union_n[name] += food[name]
        except KeyError:
            pass
    union_n['measurement_description'] += f"{food['measurement_description']},"
    union_n['total_serving_amount(g)'] += food['metric_serving_amount']


def run():
    """Запуск метода оценки энергетической ценности меню"""
    ...
    union_nutritional = {
        ...
        'total_serving_amount(g)': 0,
    }

    for food in input_foods:
        get_info_cup_of(food_name=food, union_n=union_nutritional)
